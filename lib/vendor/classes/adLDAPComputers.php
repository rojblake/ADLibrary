...
class adLDAPComputers {
    ...
    */
    ...
    /**
    * Returns a complete list of all computers in AD
    * 
    * @param bool $includeDistinguishedName Whether to return a full distinquished name
    * @param string $search Search parameters
    * @param bool $sorted Whether to sort the results
    * @return array
    */
    public function all($includeDistinguishedName = false, $search = "*", $sorted = true)
	{
        if (!$this->adldap->getLdapBind()) { return false; }
        
        // Perform the search and grab all their details
        $filter = "(&(objectClass=computer)(samaccounttype=" . adLDAP::ADLDAP_WORKSTATION_TRUST .")(objectCategory=Computer)(cn=" . $search . "))";
        $fields = array("name","distinguishedname");
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);

        $computersArray = array();
        for ($i=0; $i<$entries["count"]; $i++){
            if ($includeDistinguishedName && strlen($entries[$i]["distinguishedname"][0])>0){
                $computersArray[$entries[$i]["name"][0]] = $entries[$i]["distinguishedname"][0];
            } elseif ($includeDistinguishedName){
                $computersArray[$entries[$i]["name"][0]] = $entries[$i]["name"][0];
            } else {
                array_push($computersArray, $entries[$i]["name"][0]);
            }
        }
        if ($sorted) { 
            asort($computersArray); 
        }
